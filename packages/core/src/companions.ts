...
export function isRecruited(state: GameState, companionId: string): boolean {
  return !!state.flags[`companion_${companionId}`];
}

/** 当前同伴的防御协防总值（计入天灾判定） */
export function getCompanionDefense(state: GameState): number {
  return COMPANION_DEFS.filter((c) => isRecruited(state, c.id)).reduce((sum, c) => sum + c.defense, 0);
}

/** 当前同伴的排行榜声望加成 */
export function getCompanionRankBonus(state: GameState): number {
  return COMPANION_DEFS.filter((c) => isRecruited(state, c.id)).reduce((sum, c) => sum + c.rankBonus, 0);
}

/** 招募同伴：好感达标后写入 companion_* 标记，返回招募播报。 */
export function recruitCompanion(state: GameState, companionId: string, currentFavor: number): string | null {
  const def = COMPANION_DEFS.find((c) => c.id === companionId);
  if (!def) return null;
  if (isRecruited(state, companionId)) return null;
  if (currentFavor < def.recruitFavor) return null;
  state.flags[`companion_${companionId}`] = true;
  return `【同伴】${def.joinText}`;
}

/** 每日同伴被动（runDaily 调用） */
export function applyCompanionDaily(state: GameState): string[] {
  const messages: string[] = [];
  for (const c of COMPANION_DEFS) {
    if (!isRecruited(state, c.id) || !c.daily) continue;
    const msg = c.daily(state);
    if (msg) messages.push(`【同伴】${msg}`);
  }
  return messages;
}
