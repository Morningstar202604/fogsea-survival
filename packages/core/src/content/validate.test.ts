...
function reachableStoryScenes(): Set<string> {
  const scenes = fullContent.storyline.scenes;
  const roots = new Set<string>([fullContent.storyline.initialScene, ...PHASE_STORY.map((b) => b.entryScene)]);
  ...
  while (queue.length) {
    ...
    for (const c of node.choices ?? []) {
      ...
      if (c.next && c.next !== '__return__') targets.push(c.next);
      for (const e of c.effects ?? []) {
        if (e.kind === 'jump' && e.target) targets.push(e.target);
        if (e.onSuccess) targets.push(e.onSuccess);
        if (e.onFail) targets.push(e.onFail);
      }
      for (const t of targets) {
        if (scenes[t] && !seen.has(t)) { seen.add(t); queue.push(t); }
      }
    }
  }
  return seen;
...
describe('content validator', () => {
  ...
  it('每条阶段剧情链从入口出发必须存在出口（回到枢纽或结束事件）', () => {
    ...
    for (const beat of PHASE_STORY) {
      const seen = new Set<string>([beat.entryScene]);
      ...
      while (queue.length) {
        ...
        if (!node) continue;
        for (const c of node.choices ?? []) {
          if (c.next === 'start' || c.next === '__return__') hasExit = true;
          const t = c.next && c.next !== '__return__' && c.next !== 'start' ? c.next : null;
          if (t && scenes[t] && !seen.has(t)) { seen.add(t); queue.push(t); }
        }
      }
      if (!hasExit) {
