...
describe('v2.0 Integration Tests', () => {
  ...
  test('should produce resources from base buildings', () => {
    ...
    const upgradeResult = upgradeBase(state as any);
  ...
  test('should unlock and level up skills', () => {
    ...
    // 选择专精
    const specResult = chooseSpecialization(state as any, SkillBranch.TECHNOLOGY);
    expect(specResult.success).toBe(true);
    expect(state.skills.specialization).toBe(SkillBranch.TECHNOLOGY);
    
    // 解锁科技系技能
    const unlockResult = unlockSkill(state as any, 'weapon_master');
    expect(unlockResult.success).toBe(true);
    expect(state.skills.skills['weapon_master'].level).toBe(1);
    
    // 再次升级
    const upgradeResult = unlockSkill(state as any, 'weapon_master');
    expect(upgradeResult.success).toBe(true);
  ...
  test('should calculate skill bonuses correctly', () => {
    ...
    // 解锁一些技能
    state.skills.points = 10;
    unlockSkill(state as any, 'explorer');
    unlockSkill(state as any, 'survival_expert');
    
    // 计算技能加成
    const bonuses = calculateSkillBonuses(state as any);
