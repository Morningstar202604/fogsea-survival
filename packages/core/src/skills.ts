...
export interface SkillTreeState {
  points: number;           // 可用技能点
  totalPoints: number;      // 累计获得的技能点
  skills: Record<string, SkillState>;
  specialization?: SkillBranch; // 已选择的专精
  canChooseSpecialization: boolean; // 是否可以选择专精
}
...
/** 科技系技能定义 */
export const TECHNOLOGY_SKILLS: SkillDef[] = [
  {
    id: 'weapon_master',
    name: '武器大师',
...
/** 修仙系技能定义 */
export const CULTIVATION_SKILLS: SkillDef[] = [
  {
    id: 'talisman_maker',
    name: '符箓师',
 ...
 * 创建初始技能树状态
 */
export function createInitialSkillTree(): SkillTreeState {
  return {
    points: 0,
    totalPoints: 0,
    skills: {},
 ...
 * 获得技能点（通过升级、完成任务等）
 */
export function gainSkillPoints(
  state: GameState & { skills: SkillTreeState },
  amount: number,
): void {
...
export function calculateSkillBonuses(
  ...
  for (const [skillId, skillState] of Object.entries(state.skills.skills)) {
    if (!skillState.unlocked || skillState.level === 0) continue;
 ...
 * 应用单个加成
 */
function applyBonus(bonuses: SkillBonuses, key: string, value: any): void {
  switch (key) {
    // 战斗类
    case 'combat_damage':
      bonuses.combat.damageMultiplier += value as number;
      break;
    case 'crit_chance':
      bonuses.combat.critChance += value as number;
      break;

    // 生存类
    case 'resource_consumption':
      bonuses.survival.resourceConsumptionReduction += (1 - (value as number));
      break;
    case 'natural_recovery':
      bonuses.survival.recoveryBonus += (value as number) - 1;
      break;
    case 'fog_disperse_range':
    case 'movement_speed':
      bonuses.survival.explorationSpeed += value as number;
      break;

    // 经济类
    case 'trade_fee_reduction':
      bonuses.economy.tradeDiscount += value as number;
      break;
    case 'auto_production':
    case 'building_speed':
      bonuses.economy.productionBonus += value as number;
      break;
 ...
 * 获取分支中文名
 */
function getBranchName(branch: SkillBranch): string {
...
export function getAvailableSkills(
  state: GameState & { skills: SkillTreeState },
): Array<{ skill: SkillDef; canUnlock: boolean; reason?: string }> {
  const result: Array<{ skill: SkillDef; canUnlock: boolean; reason?: string }> = [];
