...
(function (BaseType) {
    BaseType["HUT"] = "hut";
    BaseType["WOODEN_HOUSE"] = "wooden_house";
    BaseType["STONE_HOUSE"] = "stone_house";
    BaseType["FORTRESS"] = "fortress";
    BaseType["VALLEY_BASE"] = "valley_base";
})(BaseType || (BaseType = {}));
/** 建筑类型枚举 */
...
(function (StructureType) {
    ...
    StructureType["LOGGING_CAMP"] = "logging_camp";
    // 防御类
    StructureType["WOODEN_SPIKE"] = "wooden_spike";
    StructureType["FENCE"] = "fence";
    StructureType["BALLISTA_TOWER"] = "ballista_tower";
    StructureType["WALL"] = "wall";
    StructureType["WATCHTOWER"] = "watchtower";
    // 功能类
    StructureType["STORAGE"] = "storage";
    StructureType["WELL"] = "well";
    StructureType["FIRE_PIT"] = "fire_pit";
    StructureType["MEDICAL_TENT"] = "medical_tent";
 ...
 * 建造建筑
 */
export function buildStructure(state, structureId, position) {
 ...
 * 处理每日生产
 */
export function processDailyProduction(state) {
    const produced = {};
    const messages = [];
    for (const structure of state.base.structures) {
        ...
        for (const effect of productionEffects) {
            if (effect.target && effect.value > 0) {
                ...
                if (RESOURCE_KEYS.includes(effect.target) && state.resources[effect.target]) {
                    deltaResource(state.resources[effect.target], effect.value);
                }
                else {
 ...
 */
export function recalcBaseDefense(base) {
    recalculateDefense(base);
}
/**
 * 重新计算基地总防御力
 */
function recalculateDefense(base) {
    let totalDefense = 0;
 ...
 * 处理兽潮攻击
 */
export function handleBeastWave(state, wavePower) {
    const messages = [];
 ...
 * 损坏基地建筑
 */
function damageBaseStructures(base, damage) {
    let damagedCount = 0;
    // 按防御值从低到高损坏
