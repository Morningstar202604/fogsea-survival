...
import { createInitialState, runDaily, applyChoice, applyEventChoice, resolveScene, availableChoices, resolveEvent, executeCombatRound, evaluateCatastrophe, resolveCatastrophe, Rng, } from './index.js';
...
describe('天灾结算（需求判定 + 奖惩落地）', () => {
    ...
    it('准备充分判定成功，奖励落地（XP→技能点、物品）', () => {
        const s = createInitialState(fullContent);
        s.base.level = 5;
        s.base.totalDefense = 500;
        s.skills.points = 0;
        const verdict = evaluateCatastrophe(s, wave1);
        expect(verdict.success).toBe(true);
        const before = s.inventory['beast_core'] ?? 0;
        resolveCatastrophe(s, wave1, true);
    ...
    it('失败惩罚落地：资源、物品、生命三重损失', () => {
        ...
        s.inventory['wood'] = 60;
        resolveCatastrophe(s, wave1, false);
