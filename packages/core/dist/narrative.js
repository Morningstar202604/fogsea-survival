...
// ============================================================
/** 叙事场景类型 */
export var NarrativeSceneType;
(function (NarrativeSceneType) {
    NarrativeSceneType["DIALOGUE"] = "dialogue";
    NarrativeSceneType["EXPLORATION"] = "exploration";
    NarrativeSceneType["COMBAT"] = "combat";
    NarrativeSceneType["NEGOTIATION"] = "negotiation";
    NarrativeSceneType["DILEMMA"] = "dilemma";
    NarrativeSceneType["DISCOVERY"] = "discovery";
    NarrativeSceneType["DISASTER"] = "disaster";
    NarrativeSceneType["TRADE"] = "trade";
    NarrativeSceneType["TRAINING"] = "training";
    NarrativeSceneType["STORY"] = "story";
})(NarrativeSceneType || (NarrativeSceneType = {}));
/** 叙事选择类型 */
export var NarrativeChoiceType;
(function (NarrativeChoiceType) {
    NarrativeChoiceType["ACTION"] = "action";
    NarrativeChoiceType["DIALOGUE"] = "dialogue";
    NarrativeChoiceType["COMBAT"] = "combat";
    NarrativeChoiceType["NEGOTIATION"] = "negotiation";
    NarrativeChoiceType["ITEM_USE"] = "item_use";
    NarrativeChoiceType["SKILL_USE"] = "skill_use";
    NarrativeChoiceType["FLEE"] = "flee";
    NarrativeChoiceType["OBSERVE"] = "observe";
    NarrativeChoiceType["WAIT"] = "wait";
    NarrativeChoiceType["GIVE_UP"] = "give_up";
})(NarrativeChoiceType || (NarrativeChoiceType = {}));
/** 叙事结果类型 */
export var NarrativeOutcomeType;
(function (NarrativeOutcomeType) {
    NarrativeOutcomeType["SUCCESS"] = "success";
    NarrativeOutcomeType["FAILURE"] = "failure";
    NarrativeOutcomeType["PARTIAL_SUCCESS"] = "partial_success";
    NarrativeOutcomeType["CATASTROPHIC_FAILURE"] = "catastrophic_failure";
    NarrativeOutcomeType["UNEXPECTED_GAIN"] = "unexpected_gain";
    NarrativeOutcomeType["NEUTRAL"] = "neutral";
})(NarrativeOutcomeType || (NarrativeOutcomeType = {}));
// ============================================================
// 二、叙事引擎核心类
// ============================================================
export class NarrativeEngine {
     ...
     * 处理玩家选择
     */
    processChoice(state, scene, choice) {
        const result = {
            success: false,
    ...
    determineSceneType(state) {
        ...
        if (state.attributes.strength > 15)
            weights[NarrativeSceneType.COMBAT] += 10;
        if (state.attributes.intelligence > 15)
            weights[NarrativeSceneType.NEGOTIATION] += 10;
        if (state.attributes.agility > 15)
            weights[NarrativeSceneType.EXPLORATION] += 10;
        if (state.resources.health.current < 50)
            weights[NarrativeSceneType.DISASTER] += 15;
        if (state.resources.sanity.current < 50)
            weights[NarrativeSceneType.DILEMMA] += 10;
        if (state.day > 30)
            weights[NarrativeSceneType.COMBAT] += 5;
        if (state.day > 60)
            weights[NarrativeSceneType.NEGOTIATION] += 5;
     ...
     * 生成失败文本
     */
    generateFailureText(_choice, outcome) {
     ...
     * 应用后果
     */
    applyConsequences(state, consequences, result) {
            ...
            switch (consequence.type) {
                case 'resource':
                    if (consequence.resource) {
                        ...
                        state.resources[consequence.resource].current = Math.max(0, Math.min(max, current + (consequence.amount ?? 0)));
                        result.systemMessages.push(`${consequence.resource === 'health' ? '生命' :
                ...
                case 'item':
                    if (consequence.itemId) {
                        ...
                        result.systemMessages.push(`获得物品: ${consequence.itemId} x${consequence.amount ?? 1}`);
                    }
                    break;
                case 'flag':
                    if (consequence.flag) {
     ...
     * 应用消耗
     */
    applyCosts(state, choice) {
        for (const cost of choice.costs) {
            switch (cost.type) {
                case 'resource':
                    if (cost.resource) {
                        state.resources[cost.resource].current = Math.max(0, state.resources[cost.resource].current - cost.amount);
                    }
                    break;
                case 'item':
                    if (cost.itemId) {
                        state.inventory[cost.itemId] = Math.max(0, (state.inventory[cost.itemId] ?? 0) - cost.amount);
                    }
                    break;
                case 'ap':
                    ...
                    break;
                case 'health':
                    state.resources.health.current = Math.max(0, state.resources.health.current - cost.amount);
                    break;
                case 'sanity':
                    state.resources.sanity.current = Math.max(0, state.resources.sanity.current - cost.amount);
                    break;
                case 'energy':
                    state.resources.energy.current = Math.max(0, state.resources.energy.current - cost.amount);
                    break;
            }
        }
    }
    ...
    applyEmotionalImpact(state, impact) {
        if (impact.fear) {
            state.resources.sanity.current = Math.max(0, Math.min(100, state.resources.sanity.current - impact.fear));
        }
        if (impact.hope) {
            state.resources.sanity.current = Math.max(0, Math.min(100, state.resources.sanity.current + impact.hope));
        }
        if (impact.despair) {
            state.resources.sanity.current = Math.max(0, Math.min(100, state.resources.sanity.current - impact.despair));
        }
    }
    /**
     * 确定失败结果类型
     */
    determineFailureOutcome(roll, successChance) {
     ...
     * 创建幻觉场景
     */
    createHallucinationScene(_state) {
        return {
     ...
     * 创建随机特殊事件
     */
    createRandomSpecialEvent(_state) {
        return {
    ...
    generateSceneText(_type, _state) {
        return `你面临一个新的情况...`;
    }
    generateChoices(_type, _state) {
        return [];
    }
    generateEnvironment(_type, _state) {
        return '周围的环境描述...';
    }
}
