...
import { createInitialState, applyChoice, availableChoices, conditionMet, resolveScene, findEnding, } from './engine.js';
import { serialize, deserialize, exportState, importState, } from './save.js';
import { Rng } from './rng.js';
/** 在内容包里找到第一个带 requires 的选项（demo/full 均可） */
function firstRequiringChoice(content) {
    for (const [id, node] of Object.entries(content.storyline.scenes)) {
        const c = node.choices.find((x) => x.requires);
        if (c)
            return { choice: c, sceneId: id };
    }
...
describe('core engine edge cases', () => {
    ...
    it('availableChoices 依据 requires 过滤选项', () => {
        ...
        if (!hit)
            throw new Error('fullContent 未找到带 requires 的选项，测试前置失效');
        ...
        const passState = structuredClone(s);
        ...
        const failState = structuredClone(s);
    ...
    it('支线 __return__ 出栈返回上级场景', () => {
        ...
        const c = { id: 'test_ret', text: '返回', effects: [], next: '__return__' };
        applyChoice(demoContent, s, c, new Rng(1));
    ...
    it('跳转结局 id 时写入 outcome', () => {
        ...
        const c = { id: 'test_end', text: '走向结局', effects: [], next: eid };
...
//# sourceMappingURL=engine.core.test.js.map