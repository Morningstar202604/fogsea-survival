...
function allSceneIds(content) {
    const ids = new Set(Object.keys(content.storyline.scenes));
    for (const line of content.lines ?? [])
        for (const id of Object.keys(line.scenes))
            ids.add(id);
    return ids;
}
function endingIds(content) {
    const ids = new Set(Object.keys(content.storyline.endings));
    for (const line of content.lines ?? [])
        for (const id of Object.keys(line.endings ?? {}))
            ids.add(id);
    return ids;
}
/** 文本清洗质量检查：换行残尾 / 模板占位 / markdown 痕迹 / 控制字符 */
function textLeakIssues(text, path) {
    const out = [];
    if (text.includes('{') && text.includes('}')) {
        const m = text.match(/\{[^}\n]{1,40}\}/);
        if (m)
            out.push({ code: 'V12', path, msg: `疑似模板占位未解析: ${m[0]}` });
    }
    if (/\*\*|^#+\s|`|\[.*\]\(/.test(text)) {
...
/** 校验单个选项 */
function validateChoice(choice, path, sceneIdSet, endingIdSet, issues, seenChoiceIds) {
    if (!choice.id) {
    ...
    if (!choice.text)
        issues.push({ code: 'V1', path, msg: '选项缺少文案' });
    // next 可达性
        ...
        switch (eff.kind) {
            case 'resource':
                ...
                if (typeof eff.delta !== 'number')
                    issues.push({ code: 'V4', path: ep, msg: 'resource delta 非数字' });
                break;
            case 'flag':
                if (!eff.flag)
                    issues.push({ code: 'V7', path: ep, msg: 'flag 效果缺少 flag 名' });
                break;
            case 'item':
                if (!eff.item)
                    issues.push({ code: 'V7', path: ep, msg: 'item 效果缺少 item id' });
                break;
            case 'roll': {
                const rd = eff.difficulty ?? 50;
                if (rd < 0 || rd > 100)
                    issues.push({ code: 'V10', path: ep, msg: `检定难度越界: ${rd}` });
            ...
            case 'combat':
                ...
                break;
            default:
                issues.push({ code: 'V3', path: ep, msg: `未知效果类型: ${String(eff.kind)}` });
        }
    }
    // 条件引用
    if (choice.requires) {
    ...
    // 文本泄漏
    if (choice.text)
        issues.push(...textLeakIssues(choice.text, `${path}->${choice.id}.text`));
    if (choice.result)
        issues.push(...textLeakIssues(choice.result, `${path}->${choice.id}.result`));
    if (choice.hint)
        issues.push(...textLeakIssues(choice.hint, `${path}->${choice.id}.hint`));
}
/** 校验单个场景节点 */
function validateSceneNode(node, path, sceneIdSet, endingIdSet, issues, isMainline) {
    if (!node.text)
        issues.push({ code: 'V1', path, msg: '场景缺少正文' });
    else
        issues.push(...textLeakIssues(node.text, `${path}.text`));
    const seen = new Set();
    ...
    if (!isMainline && node.choices.length > 0) {
        ...
        if (allSelf)
            issues.push({ code: 'V11', path, msg: `节点所有选项均指向自身（死循环，玩家无法离开）` });
    }
}
/** 校验结局定义 */
function validateEnding(e, path, issues) {
    if (!e.title)
        issues.push({ code: 'V8', path, msg: '结局缺少标题' });
    if (!e.desc)
        issues.push({ code: 'V8', path, msg: '结局缺少描述' });
    else
        issues.push(...textLeakIssues(e.desc, `${path}.desc`));
}
/** 全量校验一个 ContentPack，返回错误清单（空数组=通过） */
export function validateContentPack(content) {
    const issues = [];
    const scenes = allSceneIds(content);
    const endings = endingIds(content);
    // 版本
    if (typeof content.version !== 'string' || !/^\d+\.\d+\.\d+$/.test(content.version))
        issues.push({ code: 'V1', path: 'version', msg: 'version 非法，应为 x.y.z 格式' });
    // 主线
    ...
    for (const [id, node] of Object.entries(story.scenes)) {
        ...
        if (mainSeen.has(id))
            issues.push({ code: 'V5', path: `storyline.${id}`, msg: `主线场景 id 重复: ${id}` });
        mainSeen.add(id);
    }
    for (const [id, e] of Object.entries(story.endings))
        validateEnding(e, `storyline.endings.${id}`, issues);
    // 支线
    const lineSeen = new Set();
    for (const line of content.lines ?? []) {
        if (lineSeen.has(line.id))
            issues.push({ code: 'V5', path: `lines.${line.id}`, msg: `支线 id 重复: ${line.id}` });
        ...
        const lineSeenIds = new Set();
        for (const [id, node] of Object.entries(line.scenes)) {
            if (lineSeenIds.has(id))
                issues.push({ code: 'V5', path: `lines.${line.id}.${id}`, msg: `场景 id 重复: ${id}` });
            lineSeenIds.add(id);
    ...
    for (const ev of content.randomEvents) {
        if (evSeen.has(ev.id))
            issues.push({ code: 'V5', path: `randomEvents.${ev.id}`, msg: `事件 id 重复: ${ev.id}` });
        evSeen.add(ev.id);
        ...
        if (ev.text)
            issues.push(...textLeakIssues(ev.text, `randomEvents.${ev.id}.text`));
        const seen = new Set();
