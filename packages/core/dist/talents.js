export const TALENTS = [
    {
        ...
        deepEffects: [
            { description: '物资嗅觉增强：食物+20，水+15', apply: (s) => {
    ...
    },
    {
        ...
        deepEffects: [
            { description: '幸运强化：幸运+3', apply: (s) => {
                    s.attributes.luck += 3;
                    return ['幸运强化：幸运+3'];
                } },
            { description: '幸运直觉：暴击率+5%', apply: (s) => {
                    s.flags['talent_lucky_star_crit'] = true;
                    return ['幸运直觉：暴击率+5%'];
                } },
            { description: '幸运眷顾：稀有物品掉落率+10%', apply: (s) => {
                    s.flags['talent_lucky_star眷顾'] = true;
                    return ['幸运眷顾：稀有物品掉落率+10%'];
                } },
            { description: '幸运守护：死亡时有几率复活', apply: (s) => {
                    s.flags['talent_lucky_star_guard'] = true;
                    return ['幸运守护：死亡时有几率复活'];
                } },
            { description: '幸运光环：队友幸运+5', apply: (s) => {
...
export function drawTalentChoices(rng) {
    const pool = [...TALENTS];
    const picked = [];
...
export function applyTalent(state, talentId) {
    if (state.flags['talent_chosen'])
        return [];
...
// ============================================================
/** 初始化天赋深精状态 */
export function initTalentDeepState() {
    return {};
}
/** 获取天赋深精等级 */
export function getTalentDeepLevel(state, talentId) {
    const deepState = state.talentDeepState?.[talentId];
    return deepState?.deepLevel ?? 0;
}
/** 检查天赋是否可以深精 */
export function canDeepTalent(state, talentId) {
    const talent = TALENTS.find((t) => t.id === talentId);
    if (!talent)
        return false;
    if (!talent.deepEffects)
        return false;
...
/** 计算深精成本（指数级增加） */
export function calculateDeepCost(talentId, currentLevel) {
    const talent = TALENTS.find((t) => t.id === talentId);
    if (!talent)
        return 0;
...
/** 执行天赋深精 */
export function deepTalent(state, talentId) {
    if (!canDeepTalent(state, talentId)) {
        return ['无法进行深精：条件不满足或已达最大等级'];
    }
    const talent = TALENTS.find((t) => t.id === talentId);
    if (!talent || !talent.deepEffects)
        return [];
    const currentLevel = getTalentDeepLevel(state, talentId);
...
/** 重置天赋深精（返回一半的深精点数） */
export function resetTalentDeep(state, talentId) {
    const deepState = state.talentDeepState?.[talentId];
