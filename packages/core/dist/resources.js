...
/** 根据世界等级修正的每日消耗速率 */
export function getDailyConsumptionRate(tier) {
...
export function isValidDailyConsumption(rate) {
    return (!isNaN(rate.food) &&
        !isNaN(rate.water) &&
        !isNaN(rate.sanity) &&
        !isNaN(rate.energy));
}
/** 每日结算：按世界等级压力自动消耗食物/水/理智/体力（天赋被动在此生效） */
export function applyDailyConsumption(state) {
    const messages = [];
    ...
    const reduction = state.skills
        ? Math.min(0.5, Math.max(0, calculateSkillBonuses(state).survival.resourceConsumptionReduction))
        : 0;
    // 天赋被动修正
    const modifiers = {
