...
/** 获取阶段的大事件 */
export function getMajorEventByDay(day) {
    const phase = getPhaseByDay(day);
    if (day === phase.majorEvent.day)
        return phase.majorEvent;
    return null;
}
...
export const MIST_DENSITY_CONFIG = {
...
/** 根据天数和阶段计算每日天气 */
export function calculateDailyWeather(day, rng) {
    const phase = getPhaseByDay(day);
    const roll = rng();
    // 后期恶劣天气概率增加
    ...
    if (roll < bloodyMoonChance)
        return 'bloody_moon';
    if (roll < bloodyMoonChance + mistTideChance)
        return 'mist_tide';
    if (roll < bloodyMoonChance + mistTideChance + stormChance)
        return 'stormy';
    if (roll < bloodyMoonChance + mistTideChance + stormChance + 0.2)
        return 'rainy';
    if (roll < bloodyMoonChance + mistTideChance + stormChance + 0.2 + 0.5)
        return 'foggy';
    return 'clear';
}
/** 根据天数和天气计算迷雾浓度 */
export function calculateMistDensity(day, weather) {
    const phase = getPhaseByDay(day);
    let base = 'normal';
    if (phase.id <= 2)
        base = 'thin';
...
/** 计算危险等级 */
export function calculateDangerLevel(day, mistDensity, weather) {
...
/** 根据天数获取已解锁的区域 */
export function getUnlockedZones(day) {
...
/** 根据天数和阶段获取可遇到的NPC */
export function getAvailableNpcs(day, phase) {
    return NPC_DEFS.filter(npc => day >= npc.unlockCondition.minDay &&
...
/** 检查因果关系是否触发 */
export function checkCausalRelations(causeType, description) {
    return CAUSAL_RELATIONS.filter(c => c.cause.type === causeType &&
...
/** 计算玩家的成长方向倾向 */
export function calculateGrowthPath(state) {
    const scores = [];
...
export const ATTRIBUTE_FORMULA = {
...
export function calculateMeleeDamage(strength, weaponAttack, skillMultiplier = 1) {
...
export function calculateCombatExp(enemyLevel, victory) {
    if (!victory)
        return Math.floor(enemyLevel * 5);
    return Math.floor(enemyLevel * 20 + 10);
}
export function calculateExploreExp(day) {
...
export function calculateCombatPoints(enemyLevel, luck) {
...
/** 计算晶核价值 */
export function calculateCrystalValue(crystalLevel) {
...
export function calculateBaseUpgradeCost(currentLevel) {
    const cost = {};
...
export function calculateBaseRest(baseLevel) {
    return {
...
/** 计算基地总防御力（含建筑） */
export function calculateBaseDefense(baseLevel, buildings) {
...
export function calculateWeaponDamage(baseAttack, quality, enhanceLevel = 0) {
...
/** 随机获取装备品质（受幸运影响） */
export function rollItemQuality(luck, rng) {
    const roll = rng();
...
export function calculateDailyConsumption(day, baseConsumption, weather) {
    const phase = getPhaseByDay(day);
...
// ============================================================
export function calculateEventRate(day, mistDensity) {
    const phase = getPhaseByDay(day);
    const baseRate = 0.3;
...
// 十五、怪物强度（分阶段，保留并扩展）
// ============================================================
export function calculateMonsterStrength(day, mistDensity) {
    const phase = getPhaseByDay(day);
...
/** 评估玩家在大事件中的难度等级 */
export function assessMajorEventDifficulty(event, playerState) {
    ...
    for (const threshold of thresholds) {
        ...
        if (conditions.hasWeapon) {
            const hasWeapon = Object.keys(playerState.inventory).some(k => k.includes('spear') || k.includes('sword') || k.includes('knife') || k.includes('weapon') || k.includes('axe'));
            factors.push({ factor: '武器', value: hasWeapon ? 1 : 0, pass: hasWeapon });
            if (!hasWeapon)
                allPass = false;
        }
        if (conditions.hasArmor) {
            const hasArmor = Object.keys(playerState.inventory).some(k => k.includes('armor') || k.includes('leather') || k.includes('cloth_armor'));
...
export const SUPERPOWER_LEVELS = [
...
/** 计算超能力伤害 */
export function calculateSuperpowerDamage(powerType, powerLevel, primaryAttr) {
...
/** 计算超能力失控概率 */
export function calculate失控Chance(powerType, powerLevel, sanity) {
    const type = SUPERPOWER_TYPES.find(t => t.id === powerType);
    const level = SUPERPOWER_LEVELS.find(l => l.level === powerLevel);
    if (!type || !level)
        return 0;
...
/** 计算超能力训练经验获取 */
export function calculateTrainingExp(powerLevel, trainingHours, intelligence) {
...
/** 计算研究时间（受研究员数量和智力影响） */
export function calculateResearchTime(baseTime, researcherCount, avgIntelligence) {
...
/** 检查科技前置条件是否满足 */
export function checkTechPrerequisites(techId, researchedTechs) {
...
/** 计算贸易价格（受关系影响） */
export function calculateTradePrice(basePrice, relationship, isBuying) {
...
/** 计算关系变化（受行动影响） */
export function calculateRelationshipChange(baseChange, playerCharisma, targetTrust) {
...
export function getReputationLevel(reputation) {
    ...
    for (const level of REPUTATION_LEVELS) {
        if (reputation >= level.minValue)
            result = level;
    }
    return result;
}
/** 计算声望获取（受魅力和行为影响） */
export function calculateReputationGain(baseReputation, playerCharisma, actionType) {
    const charismaBonus = 1 + (playerCharisma - 10) * 0.02;
    let typeMultiplier = 1;
    if (actionType === 'heroic')
        typeMultiplier = 1.5;
    if (actionType === 'selfless')
        typeMultiplier = 1.3;
...
/** 计算声望效果 */
export function calculateReputationEffects(reputation) {
